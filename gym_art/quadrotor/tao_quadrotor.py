...
from gym_art.quadrotor.test_noise import Noise
...
def crazyflie_params():
    ...
    ## Motor parameters
    motor_params = {"thrust_to_weight" : 2.33, # 2.18
...
class QuadrotorDynamics(object):
    ...
    def __init__(self, model_params,
        ...
        ###############################################################
        ## Noise
        self.noise = Noise(noise_normal_position=np.array([0.02, 0.02, 0.02]), 
                           noise_uniform_position=np.array([0.02, 0.02, 0.02]), 
                           noise_normal_linear_velocity=np.array([0.05, 0.05, 0.05]), 
                           noise_uniform_linear_velocity=np.array([0.02, 0.02, 0.02]), 
                           noise_normal_theta=np.array([0.05, 0.05, 0.05]), 
                           noise_uniform_theta=np.array([0.05, 0.05, 0.05]), 
                           measurement_delay=2)
    ...
    def state_vector(self, goal):

        # add noise to the returned sv
        noisy_pos, noisy_vel, noisy_rot, noisy_omega = self.noise.add_noise(self.pos, self.vel, self.rot, self.omega)
        noisy_pos = goal - noisy_pos

        return np.concatenate([
            noisy_pos, noisy_vel, noisy_rot.flatten(), noisy_omega])
