...
def state_xyz_vxyz_R_omega(self):
    bypass = self.sense_noise.bypass
    pos = np.array(self.dynamics.pos)
    vel = np.array(self.dynamics.vel)
    rot = np.array(self.dynamics.rot)
    omega = np.array(self.dynamics.omega)
    acc = np.array(self.dynamics.accelerometer, dtype=np.float32)

    if bypass:
        pass
    else:
        # self.sense_noise.test(pos, vel, rot, omega, acc)

        self.sense_noise.add_noise(
            pos, vel, rot, omega, self.dt,
        )

        self.sense_noise.add_noise_acc(acc)
