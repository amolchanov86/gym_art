...
class Goal(object):
    """
    Full goal state: pos, vel, omega
    For tracking trajectories that specify the vel and omega
    """
    def __init__(self, pos, vel=np.zeros(3), omega=np.zeros(3)):
        """
        Args:
            pos: np.ndarray
            vel: np.ndarray
...
def compute_reward_weighted(dynamics, goal, action, dt, crashed, time_remain, rew_coeff, action_prev):
    ...
    dist = np.linalg.norm(goal.pos - dynamics.pos)
...
class QuadrotorEnv(gym.Env, Serializable):
    ...
    def state_xyz_vxyz_rot_omega(self):        
        ...
        return np.concatenate([pos - self.goal.pos, vel - self.goal.vel, rot.flatten(), omega - self.goal.omega])
    ...
    def state_xyz_vxyz_rot_omega_h(self):        
        ...
        return np.concatenate([pos - self.goal.pos, vel - self.goal.vel, rot.flatten(), omega - self.goal.omega, (pos[2],)])
    ...
    def state_xyzr_vxyzr_rot_omega(self):
        ...
        e_xyz_rel = self.dynamics.rot.T @ (pos - self.goal.pos)
        vel_rel = self.dynamics.rot.T @ (vel - self.goal.vel)
        return np.concatenate([e_xyz_rel, vel_rel, rot.flatten(), omega - self.goal.omega])
    ...
    def state_xyzr_vxyzr_rot_omega_h(self):
        ...
        e_xyz_rel = self.dynamics.rot.T @ (pos - self.goal.pos)
        vel_rel = self.dynamics.rot.T @ (vel - self.goal.vel)
        return np.concatenate([e_xyz_rel, vel_rel, rot.flatten(), omega - self.goal.omega, (pos[2],)])
    ...
    def state_xyz_vxyz_rot_omega_acc_act(self):        
        ...
        # return np.concatenate([pos - self.goal[:3], vel, rot.flatten(), omega, (pos[2],)])
        return np.concatenate([pos - self.goal.pos, vel - self.goal.vel, rot.flatten(), omega - self.goal.omega, acc, self.actions[1]])
    ...
    def state_xyz_vxyz_rot_omega_act(self):        
        ...
        # return np.concatenate([pos - self.goal[:3], vel, rot.flatten(), omega, (pos[2],)])
        return np.concatenate([pos - self.goal.pos, vel - self.goal.vel, rot.flatten(), omega - self.goal.omega, self.actions[1]])
    ...
    def state_xyz_vxyz_quat_omega(self):
        ...
        return np.concatenate([pos - self.goal.pos, vel - self.goal.vel, quat, omega - self.goal.omega])
    ...
    def state_xyzr_vxyzr_quat_omega(self):
        ...
        e_xyz_rel = self.dynamics.rot.T @ (pos - self.goal.pos)
        vel_rel = self.dynamics.rot.T @ (vel - self.goal.vel)
        return np.concatenate([e_xyz_rel, vel_rel, quat, omega - self.goal.omega])
    ...
    def state_xyzr_vxyzr_quat_omega_h(self):
        ...
        e_xyz_rel = self.dynamics.rot.T @ (pos - self.goal.pos)
        vel_rel = self.dynamics.rot.T @ (vel - self.goal.vel)
        return np.concatenate([e_xyz_rel, vel_rel, quat, omega - self.goal.omega, (pos[2],)])
    ...
    def state_xyz_vxyz_euler_omega(self):
        ...
        return np.concatenate([pos - self.goal.pos, vel - self.goal.vel, euler, omega - self.goal.omega])
    ...
    def _reset(self):
        ...
        if self.resample_goal:
            self.goal = Goal(np.array([0., 0., np.random.uniform(0.5, 2.0)]), np.zeros(3), np.zeros(3))
        else:
            self.goal = Goal(np.array([0., 0., 2.]), np.zeros(3), np.zeros(3))
        ...
        x, y, z = self.np_random.uniform(-self.box, self.box, size=(3,)) + self.goal.pos
        ...
        self.scene.reset(self.goal.pos, self.dynamics)
    ...
    def _render(self, mode='human', close=False):
        return self.scene.render_chase(dynamics=self.dynamics, goal=self.goal.pos, mode=mode)
